...
local function PressKey(key, duration)
    CAS:BindAction("PressKey_"..key.Name, function() return Enum.ContextActionResult.Pass end, false, key)
    CAS:ActivateAction("PressKey_"..key.Name)
    task.wait(duration or 0.04)
    CAS:UnbindAction("PressKey_"..key.Name)
    return true
end

local function PressCombo(keys, duration)
    for _, key in ipairs(keys) do
        CAS:BindAction("PressCombo_"..key.Name, function() return Enum.ContextActionResult.Pass end, false, key)
        CAS:ActivateAction("PressCombo_"..key.Name)
    end
    task.wait(duration or 0.08)
    for _, key in ipairs(keys) do
        CAS:UnbindAction("PressCombo_"..key.Name)
    end
    return true
end
...
local function PressDash(directionKey, duration)
    -- Hold movement key, then press dash key (Q) to ensure dash registers
    CAS:BindAction("PressDashDir_"..directionKey.Name, function() return Enum.ContextActionResult.Pass end, false, directionKey)
    CAS:ActivateAction("PressDashDir_"..directionKey.Name)
    task.wait(0.02)
    CAS:BindAction("PressDashQ", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.Q)
    CAS:ActivateAction("PressDashQ")
    task.wait(duration or 0.12)
    CAS:UnbindAction("PressDashQ")
    CAS:UnbindAction("PressDashDir_"..directionKey.Name)
    return true
end

local function PressLMB()
    CAS:BindAction("PressLMB", function() return Enum.ContextActionResult.Pass end, false, Enum.UserInputType.MouseButton1)
    CAS:ActivateAction("PressLMB")
    task.wait(0.04)
    CAS:UnbindAction("PressLMB")
    return true
end
...
local function ReleaseAllMovement()
    local keys = {Enum.KeyCode.W, Enum.KeyCode.A, Enum.KeyCode.S, Enum.KeyCode.D}
    for _, key in ipairs(keys) do
        CAS:UnbindAction("ReleaseMove_"..key.Name)
    end
end
...
local function WASDStrafing(delta)
    ...
    if distance > GS("ApproachDistance") + 2 then
        CAS:BindAction("MoveW", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.W)
        CAS:ActivateAction("MoveW")

    elseif distance < GS("RetreatDistance") then
        CAS:BindAction("MoveS", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.S)
        CAS:ActivateAction("MoveS")

    else
        if GS("StrafeForward") then
            CAS:BindAction("MoveW2", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.W)
            CAS:ActivateAction("MoveW2")
        end
    end
    ...
    if GS("AutoStrafe") then
        ...
        if rnd(1, 100) <= GS("StrafeChance") then
            if StrafeDirection == "left" and GS("StrafeLeft") then
                if CheckWall(Vector3.new(-1, 0, 0)) then
                    CAS:BindAction("MoveA", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.A)
                    CAS:ActivateAction("MoveA")
                else
                    CAS:BindAction("MoveD", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.D)
                    CAS:ActivateAction("MoveD")
                end

            elseif StrafeDirection == "right" and GS("StrafeRight") then
                if CheckWall(Vector3.new(1, 0, 0)) then
                    CAS:BindAction("MoveD2", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.D)
                    CAS:ActivateAction("MoveD2")
                else
                    CAS:BindAction("MoveA2", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.A)
                    CAS:ActivateAction("MoveA2")
                end
            end
        end
    end
    ...
    if GS("CircleStrafe") and distance <= GS("OptimalFightDistance") * 1.5 then
        ...
        if strafePattern > GS("StrafeIntensity") then
            if CheckWall(Vector3.new(-1, 0, 0)) then
                CAS:BindAction("MoveA3", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.A)
                CAS:ActivateAction("MoveA3")
            end
        elseif strafePattern < -GS("StrafeIntensity") then
            if CheckWall(Vector3.new(1, 0, 0)) then
                CAS:BindAction("MoveD3", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.D)
                CAS:ActivateAction("MoveD3")
            end
        end
    end
end
...
local function PerformActionByName(a)
    ...
    elseif a == "dash" then return SmartDash()
    elseif a == "jump" then return Jump()
    elseif a == "strafe_left" then
        CAS:BindAction("StrafeLeft", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.A)
        CAS:ActivateAction("StrafeLeft")
        task.wait(0.08)
        CAS:UnbindAction("StrafeLeft")
        return true
    elseif a == "strafe_right" then
        CAS:BindAction("StrafeRight", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.D)
        CAS:ActivateAction("StrafeRight")
        task.wait(0.08)
        CAS:UnbindAction("StrafeRight")
        return true
    elseif a == "none" then return false
    end
    return false
...
local function StartAI()
    ...
    if GS("AlwaysShiftlock") then
        CAS:BindAction("ShiftLock", function() return Enum.ContextActionResult.Pass end, false, Enum.KeyCode.LeftShift)
        CAS:ActivateAction("ShiftLock")
    end
...
local function StopAI()
    ...
    for _, key in ipairs(otherKeys) do
        CAS:UnbindAction("StopKey_"..key.Name)
    end

    CAS:UnbindAction("StopMouse")
